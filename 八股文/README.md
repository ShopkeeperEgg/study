...
懒加载: 推迟加载，当模块出现在页面时再加载，可以节省带宽，提高页面的访问速度

预加载：对未出现在页面的元素预先加载，这样当模块出现时，图片会立即展示，以提升用户的体验
...
两个都代表鼠标移入元素时触发的事件，mouseenter 不支持冒泡

mouseenter：鼠标进入元素时，会触发mouseenter事件

mouseover：鼠标进入元素时、鼠标进入元素的子元素时，都会触发mouseover事件

https://qianlongo.github.io/zepto-analysis/example/event/mouseEnter-mouseOver.html

## 125. js 拖拽功能的实现

记录拖拽起点坐标，拖拽时计算指针或手指偏移距离，实时改变所需拖拽的元素位置
...
自答：
const 声明的变量是只读的，不可重新赋值

不存在变量提升，会生成TDZ，在声明变量之前使用会出现报错
...
自答：Set是集合，没有重复的值，WeakSet只能推入引用类型
...
自答：Map和WeakMap都可以通过key取到对应的value，但WeakMap的key必须是对象
...
非路径引入：会在nodejs内置模块和node_modules里进行查找
...
promise对象是一个状态管理对象。是异步编程的解决方案。具有pending、resolve、reject三种状态。
...
class MyPromise {
  ...
  then(resCb,rejCb) {
    if (this.state === 'resolve') {
      resCb(this.finalData);
    } else if (this.state === 'reject'){
      rejCb(this.finalData);
    } else {
      this.resolveCb = resCb;
      this.rejectCb = rejCb;
    }
  }
}
...
## 140. 单例模式模式是什么？141. 策略模式是什么？142. 代理模式是什么？143. 中介者模式是什么？144. 适配器模式是什么？

单例模式保证了全局只有一个实例来被访问。比如说常用的如弹框组件的实现和全局状态的实现。(很多方法都绑定在window上，这样全局都能访问到)

策略模式主要是用来将方法的实现和方法的调用分离开，外部通过不同的参数可以调用不同的策略。我主要在 MVP 模式解耦的时候
用来将视图层的方法定义和方法调用分离。 （stat({event: 'touch'})）

代理模式是为一个对象提供一个代用品或占位符，以便控制对它的访问。比如说常见的事件代理。 （myProxy, vue2中数组方法重写）

中介者模式指的是，多个对象通过一个中介者进行交流，而不是直接进行交流，这样能够将通信的各个对象解耦。 (MVP)
...
## 146. Vue 的生命周期是什么？
自答：vue实例从初始化到销毁所经历的阶段，每个阶段对应不同的钩子

## 147. Vue 的各个生命阶段是什么？
自答：

1. beforeCreate：实例初始化之前
2. create：实例初始化，dom未渲染
3. mounted：dom渲染在页面时
4. (before)update: 实例状态更新时
5. (before)destory: 实例销毁时
...
## 148. Vue 组件间的参数传递方式？
自答：父子组件之间传参通过props和emit方式来进行。其他类型的可以通过例如 provide/inject，busevent，vuex等方式
...
自答：
computed 是计算属性，会根据定义的函数计算出所需值
watch是侦听属性，当侦听对象发生改变时会执行对应回调
...
缓存内部组件信息，当组件切换时，会保留原有的组件内部信息

## 156. vue 中 mixin 和 mixins 区别？
mixin 全局层面的混入，每个组件都会混入信息

mixins 是组件层面的混入，设置类mixins的组件会混入引入内的组件的内容
...
class ListenManage {
  ...
  addEventListener(eventName, cb) {
    if (this.listenersMap[eventName]) {
      ...
      this.listenersMap[eventName] = [cb];
    }
  }
  ...
  emitEvent(eventName, ...ext) {
    if (this.listenersMap[eventName].length) {
      this.listenersMap[eventName].forEach(item => item(...ext));
    }
  }
}

const bus = new ListenManage();
bus.addEventListener('caonima', () => {
  console.log(1111);
});
bus.addEventListener('caonima', (aaa) => {
  console.log(222 + aaa);
});
bus.addEventListener('caonima', function () {
  console.log(333);
  // console.log(this);
});

setTimeout(() => {
  bus.emitEvent('caonima', 'aaaa')
}, 3000);
```
...
```javascript
function getMax(arts) {
  let curWords = '';
  let map = {};
  for (let i = 0; i < arts.length; i++) {
    if (arts[i] === ' ') {
      if (curWords) map[curWords] = map[curWords] ? map[curWords] + 1: 1
      curWords = '';
      continue;
    }
    curWords += arts[i];
